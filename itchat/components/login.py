...
def load_login(core):
    core.login             = login                  # implement the action: login
    core.get_QRuuid        = get_QRuuid             # for QRCode
    core.get_QR            = get_QR                 # QRCodeImg and Show it
    core.check_login       = check_login            # check
    core.web_init          = web_init               # UserInfo contactList
    core.show_mobile_login = show_mobile_login      #
    core.start_receiving   = start_receiving        # receiving daemon --> sync  getMsg
    core.get_msg           = get_msg                # implement getMessage
    core.logout            = logout                 # logout
...
def login(self, enableCmdQR=False, picDir=None, qrCallback=None,
    ...
    while self.isLogging:
        ...
        while not isLoggedIn:
            ...
            # call the customed callback method[qrCallback(__call__)]
    ...
    # init: get user info and synckey
    self.web_init()
    # ...
    self.show_mobile_login()
    # ContactList update
    self.get_contact(True)
    if hasattr(loginCallback, '__call__'):# customed loginCallback method
        r = loginCallback()
    else:
        # utils.clear_screen()# clear  os.system("cls")
...
def push_login(core):
    cookiesDict = core.s.cookies.get_dict()#s=requests.Session()
    if 'wxuin' in cookiesDict:
        url = '%s/cgi-bin/mmwebwx-bin/webwxpushloginurl?uin=%s' % (
            config.BASE_URL, cookiesDict['wxuin'])# https://login.weixin.qq.com
        headers = { 'User-Agent' : config.USER_AGENT }
        r = core.s.get(url, headers=headers).json()# Method: get URL: url
        # response --> json自动编码成json数据
...
def get_QR(self, uuid=None, enableCmdQR=False, picDir=None, qrCallback=None):
    ...
    if hasattr(qrCallback, '__call__'):# qrCallback['__call__']
...
# /cgi-bin/mmwebwx-bin/login?loginicon=true&uuid=AYZr_4mkgA==&tip=0&r=1879407285&_=1552898738275
# check if login or not (scan qrcode and confim it)
# response:
    # window.code=200;
...
# handle the info from Text[/login:response]
# redict to window.redirect_uri[]
# save info{skey, wxsid, wxuin, pass_ticket}
def process_login_info(core, loginContent):
    ...
    print("login Info={0}".format(json.dumps(core.loginInfo)))
...
# POST /webwxinit
# the response is about recent contact list
def web_init(self):
    ...
    self.loginInfo['User'] = wrap_user_dict(utils.struct_friend_info(dic['User']))# 个人信息User{NickName,Signature,UserName,HeadImgUrl,Uin,}
    self.memberList.append(self.loginInfo['User'])# 保存个人信息
    self.loginInfo['SyncKey'] = dic['SyncKey']# SyncKey{List[{Key,Val}]}
...
# cgi-bin/mmwebwx-bin/synccheck?r=1552898758293&skey=%40crypt_8909546a_46941b4c4a81b9990352742923a95d6e&sid=qxzWpKTO0HshEqA2&uin=1315386831&deviceid=e045188226309627&synckey=1_682758835%7C2_682758868%7C3_682758836%7C11_682758843%7C201_1552898757%7C203_1552886163%7C1000_1552897522%7C1001_1552894634&_=1552898756581
# https://wx2.qq.com/
# cgi-bin/mmwebwx-bin/webwxsync?sid=qxzWpKTO0HshEqA2&skey=@crypt_8909546a_46941b4c4a81b9990352742923a95d6e
def start_receiving(self, exitCallback=None, getReceivingFnOnly=False):
    ...
    def maintain_loop():
        ...
        while self.alive:
            try:
                i = sync_check(self)# window.synccheck={retcode:"0",selector:"2"}
                # i= selector
                if i is None:
                    self.alive = False
                elif i == '0':
                    ...
                    msgList, contactList = self.get_msg()# webwxsync
                    if msgList:
...
def sync_check(self):
    ...
    pm = re.search(regx, r.text)# window.synccheck={retcode:"0",selector:"2"}
